...
void tl_infer_agent_init() {
    ...
    // fprintf(stdout, "------------------\n");
    // fprintf(stdout, "input->dims->size: %d\n", dims->size);
    // for (int i = 0; i < dims->size; i++) {
    //     fprintf(stdout, "input->dims->data[%d]: %d\n", i, dims->data[i]);
    // }
    // fprintf(stdout, "------------------\n");
}
...
void tl_infer_agent_feed_input(tl_infer_agent_input_t* pInput) {
    ...
    // Normalize the resized image data and feed it to the model
	...
	assert(input[i+1] >= 0.0f && input[i+1] <= 1.0f);
	assert(input[i+2] >= 0.0f && input[i+2] <= 1.0f);
